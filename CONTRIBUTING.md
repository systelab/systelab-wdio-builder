...
3. The changes can be tested in your local before publishing. The library can be generated at "..\dist\systelab-wdio-builder" by running the command "npm run build-lib". Then, you need to replace the systelab-components folder at "your project > node-modules > systelab-wdio-builder". Please, ensure the package.json matches the version that you need to test.
